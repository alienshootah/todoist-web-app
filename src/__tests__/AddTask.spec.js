...
describe('<AddTask />', () => {
    ...
    describe('Success', () => {
        ...
        it('renders the <AddTesk /> task date overlay when using onClick', () => {
            const { queryByTestId } = render(<AddTask showAddTaskMain />);
        ...
        it('hides the <AddTesk /> main when called is clicked using onClick', () => {
            const { queryByTestId } = render(<AddTask showAddTaskMain />);

            fireEvent.click(queryByTestId('show-main-action'));
            expect(queryByTestId('add-task-main')).toBeTruthy();

            fireEvent.click(queryByTestId('show-task-main-cancel'));
            expect(queryByTestId('add-task-main')).toBeFalsy();
        });

        it('hides the <AddTesk /> main when cancel is clicked using onKeyDown', () => {
            const { queryByTestId } = render(<AddTask showAddTaskMain />);

            fireEvent.keyDown(queryByTestId('show-main-action'));
            expect(queryByTestId('add-task-main')).toBeTruthy();

            fireEvent.keyDown(queryByTestId('show-task-main-cancel'));
            expect(queryByTestId('add-task-main')).toBeFalsy();
        });
        ...
        it('renders <AddTask /> and adds a task to TODAY', () => {
            ...
            fireEvent.change(queryByTestId('add-task-content'), {
                target: { value: 'I am a nw task and I am amazing!' },
            });
        ...
        it('renders <AddTask /> and adds a task with a task date NEXT_7', () => {
            useSelectedProjectValue.mockImplementation(() => ({
                selectedProject: '1'
            }));
